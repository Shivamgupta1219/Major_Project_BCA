...
//     if (!isSaved) {
//       return; // Prevent download if not saved
//     }
//     handlePrint();
//   };
...
// export default DownloadPdfButton;import { useReactToPrint } from "react-to-print";
// import { Download, Lock } from "lucide-react";

// const DownloadPdfButton = ({ targetRef, isSaved, resumeTitle }) => {
//   const handlePrint = useReactToPrint({
//     contentRef: targetRef,        // ← use contentRef (v3+) instead of content
//     documentTitle: resumeTitle || "Resume",
//     pageStyle: `
...
//           }`}
//       >
//         {isSaved ? <><Download size={18} /> Download PDF</> : <><Lock size={18} /> Save First</>}
//       </button>
//       {!isSaved && (
//         <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
//           Please save your resume before downloading
...
const DownloadPdfButton = ({ targetRef, isSaved, resumeTitle }) => {
  const handlePrint = useReactToPrint({
    contentRef: targetRef,  // ← THIS is the fix, v3 uses contentRef not content
    documentTitle: resumeTitle || "Resume",
    pageStyle: `
      @page { size: A4; margin: 0; }
      @media print {
